    ...
    class SafeExpressionParser {
        ...
        static isAllowedExpression(expression) {
            ...
            return this.isSimplePath(expr) ||
                ...
                this.isFunctionCall(expr) || // Soporte para llamadas a funciones
                this.isStringConcatenationWithFunctions(expr) || // Soporte para concatenación con funciones
        ...
        static isFunctionCall(expression) {
            // Matches function calls like t('string'), functionName('param')
            const funcPattern = /^[a-zA-Z_$][a-zA-Z0-9_$]*\(\s*'[^']*'\s*\)$/;
            return funcPattern.test(expression.trim());
        }

        static isStringConcatenationWithFunctions(expression) {
            // Matches expressions like t('error') + ': ' + error
            const expr = expression.trim();
            
            // Split by + operator but respect quotes and parentheses
            const parts = this.splitConcatenationParts(expr);
            
            if (parts.length < 2) return false;
            
            // Each part should be either a function call, string literal, or simple path
            return parts.every(part => {
                const trimmed = part.trim();
                return this.isFunctionCall(trimmed) ||
                       /^'[^']*'$/.test(trimmed) ||
                       this.isSimplePath(trimmed) ||
                       /^\d+(\.\d+)?$/.test(trimmed);
            });
        }

        static splitConcatenationParts(expression) {
            const parts = [];
            let current = '';
            let inQuotes = false;
            let parenLevel = 0;
            ...
            for (let i = 0; i < expression.length; i++) {
                ...
                if (char === "'" && (i === 0 || expression[i-1] !== '\\')) {
                    ...
                    // Found a concatenation operator at the top level
                    if (current.trim()) {
        ...
        static evaluate(expression, context) {
            ...
            if (this.isStringConcatenationWithFunctions(expr)) {
                return this.evaluateStringConcatenationWithFunctions(expr, context);
            }
            ...
            if (funcMatch) {
                ...
                if (typeof window !== 'undefined' && typeof window[funcName] === 'function') {
                    try {
                        ...
                        console.warn(`[VortexJS] Error calling function ${funcName}:`, error);
                        return '';
                    }
                }
                
                console.warn(`[VortexJS] Function ${funcName} not found`);
                return '';
            }
        ...
        static evaluateStringConcatenationWithFunctions(expression, context) {
            const parts = this.splitConcatenationParts(expression);
            ...
            return parts.map(part => {
                ...
                } else if (this.isSimplePath(trimmed)) {
                    return this.getNestedValue(context, trimmed);
